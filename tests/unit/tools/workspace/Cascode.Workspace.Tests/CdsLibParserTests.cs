...
public sealed class CdsLibParserTests
{
    ...
    public void ParseDefinePreservesQuotedPathsWithSpaces()
    {
        ...
        workspace.WriteFile(
            "cds.lib",
            $"DEFINE analog_lib \"{libraryDirectory}\"{Environment.NewLine}"
        );
    ...
    public void ParseIncludeHandlesQuotedPathsWithSpaces(string includeToken)
    {
        ...
        var includedLibraryPath = workspace.CreateDirectory(
            Path.Combine("include directory", "library location")
        );
