...
{
    private static string CreateTempDb(out Cascode.Workspace.PdkDatabase db)
    {
        var dir = Path.Combine(Path.GetTempPath(), "cascode_tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(dir);
        var path = Path.Combine(dir, "pdk.db");
        db = Cascode.Workspace.PdkDatabase.Open(path);
        return path;
    }
    ...
    {
        var dbPath = CreateTempDb(out var db);
        using (db)
        {
