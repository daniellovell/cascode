...
public class CascodeJsonConverterBasicTests
{
    [Fact]
    ...
    public void ToJsonDocument_NoElCircuit_Throws()
    {
        ...
        {
            Circuits = [new Circuit { Name = "TestML", Level = CascodeLevel.ML }],
        };

        var ex = Assert.Throws<ArgumentException>(() => CascodeJsonConverter.ToJsonDocument(doc));
    ...
    public void ToJsonDocument_SpecificCircuitNotFound_Throws()
    {
        ...
        var ex = Assert.Throws<ArgumentException>(() =>
            CascodeJsonConverter.ToJsonDocument(doc, "NonExistent")
        );
        Assert.Contains("not found", ex.Message);
    }

    [Fact]
    public void FromJson_ValidJson_ProducesCascodeDocument()
    {
        var json =
            $@"{{
        ...
        Assert.Equal(CascodeLevel.EL, doc.Circuits[0].Level);
