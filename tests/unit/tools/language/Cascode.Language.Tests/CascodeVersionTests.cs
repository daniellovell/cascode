...
public class CascodeVersionTests
{
    [Fact]
    public void AllGoldenFiles_UseCurrentVersion()
    {
        ...
        foreach (var file in casFiles)
        {
            ...
            var expected = $"VERSION {CascodeVersion.Current}";
            Assert.True(
    ...
    {
        var differentMinor = CascodeVersion.Minor + 4;
        var content =
            $"VERSION {CascodeVersion.Major}.{differentMinor}\n"
            + "circuit Test {\n  level EL\n}\n";
        var result = CascodeReader.TryParse(content);
        Assert.True(result.Success); // No error for minor mismatch
        Assert.Equal(CascodeVersion.Major, result.Document!.VersionMajor);
    ...
    {
        var differentMajor = CascodeVersion.Major + 1;
