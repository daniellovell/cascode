...
The standard library also includes finite-Q reactive primitives: `Ideal_CapacitorQ` and
`Ideal_InductorQ`. These accept `Q` and `freq` in addition to the reactive value (`C` or `L`). On
