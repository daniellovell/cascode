...
public static class ComplianceChecker
{
        ...
        BenchResult results)
    {
        foreach (var measurement in results.Measurements)
        {
            ...
            if (!string.Equals(measurement.Value.Metric, constraint.Metric, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
            ...
            {
                if (measurement.Value.Node == null || 
                    !string.Equals(measurement.Value.Node, constraint.Node, StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }
            }
