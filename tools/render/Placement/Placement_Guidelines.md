...
Enforce symmetry as best as you can, especially since this cuts down on the number of dimensions of the optimization problem. Examples of symmetric circuits are differential pairs, current mirrors, and CMFB resistors.

If the problem becomes intractable, consider identifying different building blocks within the circuit, e.g., breaking down a multi-stage amplifier into separate stages or identifying current mirrors and differential pairs, rendering them individually, and then put all of the rendered sub-blocks into the final rendering.
...
4. For every pair of non-body terminals on the same non-rail net that resolve to a direct horizontal or vertical segment at coarse-placement time, no third component may lie strictly between the two terminals along that segment unless the third component also consumes that net on a terminal that lies on the same segment. "Strictly between" excludes the endpoints.
5. This direct-connection clearance rule is terminal-based rather than anchor-based. A same-net component is only allowed between the endpoints when one of its own same-net terminals lands on the direct segment itself; merely sharing the net elsewhere on the symbol is not sufficient. For intervening checks only, NMOS and PMOS devices are treated as having an effective 3 x 3 cell footprint centered on their anchor cell (one neighboring cell in each direction).
...
7. Components closer to the input ports should be biased towards the left side while components closer to the output ports should be biased towards the right side.
