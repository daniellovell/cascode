...
/// <summary>
/// Validates Cascode circuits for emission-blocking issues that prevent SPICE generation.
/// </summary>
/// <remarks>
...
public static class EmissionValidator
{
    ...
    public static ValidationResult Validate(Circuit circuit, CascodeDocument? document = null)
    {
    ...
    )
    {
        ...
        foreach (var source in circuit.Harness.Sources)
        {
            ...
            if (port.Direction is not (PortDirection.Input or PortDirection.Io))
            {
                result.AddError(
                    "HARN-001",
                    $"Harness source '{source.Net}' must reference an input or io port, but '{source.Net}' is declared as {port.Direction.ToCascodeString()}",
                    $"harness source {source.Net}",
        ...
        foreach (var load in circuit.Harness.Loads)
        {
            ...
            if (port.Direction is not (PortDirection.Output or PortDirection.Io))
            {
                result.AddError(
                    "HARN-002",
                    $"Harness load '{load.Net}' must reference an output or io port, but '{load.Net}' is declared as {port.Direction.ToCascodeString()}",
                    $"harness load {load.Net}",
        ...
        foreach (var bias in circuit.Harness.Biases)
        {
            ...
            if (port.Direction is not (PortDirection.Input or PortDirection.Io))
            {
                result.AddError(
                    "HARN-003",
                    $"Harness bias '{bias.Net}' must reference an input or io bias port, but '{bias.Net}' is declared as {port.Direction.ToCascodeString()}",
                    $"harness bias {bias.Net}",
