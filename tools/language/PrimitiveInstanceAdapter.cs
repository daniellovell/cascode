...
public static class PrimitiveInstanceAdapter
{
    ...
    )
    {
        static bool IsSupported(string value) =>
            value is "NMOS" or "PMOS" or "Resistor" or "Capacitor" or "Inductor" or "Diode";

        if (!string.IsNullOrWhiteSpace(instance.DeclaredType) && IsSupported(instance.DeclaredType))
        {
            primitiveKind = instance.DeclaredType;
            return true;
        }

        var resolved = InstanceTargetResolver.GetReferenceName(instance.Type);
