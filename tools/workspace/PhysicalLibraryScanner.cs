        ...
        foreach (var lib in libraries)
        {
            ...
            try
            {
                ...
                foreach (var cellDir in Directory.EnumerateDirectories(libPath))
                {
                    ...
                    var views = SafeListDir(cellDir)
                        .Select(Path.GetFileName)
                        .Where(n => !string.IsNullOrWhiteSpace(n))
                        .Select(n => n!)
                        .ToList();
                    var hasLayout = views.Any(v =>
                        LayoutViews.Contains(v!, StringComparer.OrdinalIgnoreCase)
                    );
                    var hasSymbol = views.Any(v =>
                        SymbolViews.Contains(v!, StringComparer.OrdinalIgnoreCase)
                    );
                    if (!hasLayout || !hasSymbol)
                        continue;
                    ...
                    if (NameNormalization.LooksInfra(cellName))
                        tags.Add("infra");
