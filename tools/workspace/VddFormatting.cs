...
{
    private static readonly Regex TokenRegex = new("^(?<n>\\d+)v(?<f>\\d+)$", RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase);
    ...
    {
        if (string.IsNullOrWhiteSpace(voltageDomain)) return string.Empty;
    ...
    public static bool TryTokenToVolts(string token, out double volts)
    {
        ...
        if (!int.TryParse(n, NumberStyles.None, CultureInfo.InvariantCulture, out var i)) return false;
        var frac = 0.0;
        if (f.Length > 0)
        {
            if (!int.TryParse(f, NumberStyles.None, CultureInfo.InvariantCulture, out var fi)) return false;
    ...
    {
        var m = TokenRegex.Match(token ?? string.Empty);
        if (!m.Success) return token ?? string.Empty;
    ...
    public static string PrettyFromVolts(double volts)
    {
        ...
        if (!s.Contains('.')) s += ".0"; // ensure at least one decimal place
        return s + "V";
    }
}
