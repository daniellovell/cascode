...
public static class PdkDatabaseWriter
{
    ...
    public static void UpsertMatches(
        string dbPath,
    ...
    )
    {
        ...
        var pd = insert.CreateParameter();
        pd.ParameterName = "$d";
        insert.Parameters.Add(pd);
        var pm = insert.CreateParameter();
        pm.ParameterName = "$m";
        insert.Parameters.Add(pm);
        var pq = insert.CreateParameter();
        pq.ParameterName = "$q";
        insert.Parameters.Add(pq);
        var pr = insert.CreateParameter();
        pr.ParameterName = "$r";
        insert.Parameters.Add(pr);
        var pn = insert.CreateParameter();
        pn.ParameterName = "$n";
        insert.Parameters.Add(pn);
        ...
        foreach (var rec in matches)
        {
            ...
            if (!deviceId.TryGetValue(rec.DeviceCanonicalName, out var did))
                continue;
            if (!modelId.TryGetValue(rec.ModelName, out var mid))
                continue;
            pd.Value = did;
            pm.Value = mid;
            pq.Value = rec.Quality;
            pr.Value = rec.Rank;
            pn.Value = (object?)rec.Notes ?? DBNull.Value;
    ...
    public static void UpsertGeometry(
        string dbPath,
        IReadOnlyList<ModelGeometry> geometry,
        CancellationToken cancellationToken = default
    )
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        var pwmin = insert.CreateParameter();
        pwmin.ParameterName = "$wmin";
        insert.Parameters.Add(pwmin);
        var pwmax = insert.CreateParameter();
        pwmax.ParameterName = "$wmax";
        insert.Parameters.Add(pwmax);
        var plmin = insert.CreateParameter();
        plmin.ParameterName = "$lmin";
        insert.Parameters.Add(plmin);
        var plmax = insert.CreateParameter();
        plmax.ParameterName = "$lmax";
        insert.Parameters.Add(plmax);
        var pnfmin = insert.CreateParameter();
        pnfmin.ParameterName = "$nfmin";
        insert.Parameters.Add(pnfmin);
        var pnfmax = insert.CreateParameter();
        pnfmax.ParameterName = "$nfmax";
        insert.Parameters.Add(pnfmax);
        var pamn = insert.CreateParameter();
        pamn.ParameterName = "$areamin";
        insert.Parameters.Add(pamn);
        var pamx = insert.CreateParameter();
        pamx.ParameterName = "$areamax";
        insert.Parameters.Add(pamx);
        var ppmin = insert.CreateParameter();
        ppmin.ParameterName = "$pmin";
        insert.Parameters.Add(ppmin);
        var ppmax = insert.CreateParameter();
        ppmax.ParameterName = "$pmax";
        insert.Parameters.Add(ppmax);
        var pwdef = insert.CreateParameter();
        pwdef.ParameterName = "$wdef";
        insert.Parameters.Add(pwdef);
        var pldef = insert.CreateParameter();
        pldef.ParameterName = "$ldef";
        insert.Parameters.Add(pldef);
        var pnfdef = insert.CreateParameter();
        pnfdef.ParameterName = "$nfdef";
        insert.Parameters.Add(pnfdef);
        var psrc = insert.CreateParameter();
        psrc.ParameterName = "$src";
        insert.Parameters.Add(psrc);
        var pnotes = insert.CreateParameter();
        pnotes.ParameterName = "$notes";
        insert.Parameters.Add(pnotes);
        ...
        foreach (var g in geometry)
        {
            ...
            if (!modelId.TryGetValue(g.ModelName, out var mid))
                continue;
            pid.Value = mid;
    ...
    public static void UpsertDeviceGeometry(
        string dbPath,
    ...
    )
    {
        ...
        var deviceId = LoadIdMap(db.Connection, tx, "devices", "canonical_name");
        var geomByModel = modelGeometry.ToDictionary(
            g => g.ModelName,
            StringComparer.OrdinalIgnoreCase
        );
        var bestByDevice = new Dictionary<string, (int Rank, string ModelName)>(
            StringComparer.OrdinalIgnoreCase
        );
        foreach (
            var match in matches
                .OrderBy(m => m.Rank)
                .ThenBy(m => m.ModelName, StringComparer.OrdinalIgnoreCase)
        )
        {
            if (
                !bestByDevice.TryGetValue(match.DeviceCanonicalName, out var current)
                || match.Rank < current.Rank
            )
            {
        ...
        foreach (var device in devices)
        {
            if (++count % 50 == 0)
                cancellationToken.ThrowIfCancellationRequested();
            if (!deviceId.TryGetValue(device.CanonicalName, out var did))
                continue;
            if (!bestByDevice.TryGetValue(device.CanonicalName, out var best))
                continue;
            if (!geomByModel.TryGetValue(best.ModelName, out var geom))
                continue;
            ...
            var source = string.IsNullOrWhiteSpace(geom.Source)
                ? best.ModelName
                : $"{geom.Source}:{best.ModelName}";
            psrc.Value = source;
    ...
    public static void RebuildDeviceClassSummary(
        string dbPath,
        CancellationToken cancellationToken = default
    )
    {
        ...
        using (var cmd = db.Connection.CreateCommand())
        {
            ...
            while (r.Read())
            {
                ...
                if (!classVt.TryGetValue(cls, out var vtSet))
                {
                    vtSet = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    classVt[cls] = vtSet;
                }
                if (!classVdd.TryGetValue(cls, out var vddSet))
                {
                    vddSet = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    classVdd[cls] = vddSet;
                }
                if (!r.IsDBNull(1))
                    foreach (
                        var tok in r.GetString(1)
                            .Split(
                                ',',
        ...
        using (var cmd = db.Connection.CreateCommand())
        {
            ...
            using var r = cmd.ExecuteReader();
            while (r.Read())
                classMatched[r.GetInt32(0)] = r.GetInt32(1);
        }
        ...
        using (var cmd = db.Connection.CreateCommand())
        {
            ...
            using var r = cmd.ExecuteReader();
            while (r.Read())
                classAmbiguous[r.GetInt32(0)] = r.GetInt32(1);
        }
        ...
        using (var cmd = db.Connection.CreateCommand())
        {
            ...
            while (r.Read())
            {
                ...
                if (!modelsByClass.TryGetValue(cls, out var set))
                {
                    set = new HashSet<long>();
                    modelsByClass[cls] = set;
                }
                set.Add(mid);
                if (!exampleByClassTemp.TryGetValue(cls, out var names))
                {
                    names = new List<string>();
                    exampleByClassTemp[cls] = names;
                }
                names.Add(mname);
            }
            ...
            c2.Transaction = tx;
            c2.CommandText =
                @"SELECT mc.model_id, c.name FROM model_contexts mc JOIN corners c ON c.id=mc.corner_id WHERE c.name IS NOT NULL AND c.name<>''";
            ...
            while (r2.Read())
            {
                ...
                if (string.IsNullOrWhiteSpace(corner))
                    continue;
                if (!cornersByModel.TryGetValue(mid, out var set))
                {
                    set = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    cornersByModel[mid] = set;
                }
                set.Add(corner);
            }
            ...
            using (var d2 = db.Connection.CreateCommand())
            {
                ...
                while (r3.Read())
                    decksByModel[r3.GetInt64(0)] = r3.GetInt32(1);
                foreach (var (cls, mids) in modelsByClass)
                {
                    var sum = 0;
                    foreach (var mid in mids)
                        if (decksByModel.TryGetValue(mid, out var c))
                            sum += c;
                    classDecks[cls] = sum;
                }
            }
            ...
            {
                classExampleModel[cls] =
                    names.OrderBy(s => s, StringComparer.OrdinalIgnoreCase).FirstOrDefault() ?? "-";
            }
        }
        ...
        using (var ins = db.Connection.CreateCommand())
        {
            ...
            var pCls = ins.CreateParameter();
            pCls.ParameterName = "$cls";
            ins.Parameters.Add(pCls);
            var pDc = ins.CreateParameter();
            pDc.ParameterName = "$dc";
            ins.Parameters.Add(pDc);
            var pMc = ins.CreateParameter();
            pMc.ParameterName = "$mc";
            ins.Parameters.Add(pMc);
            var pAc = ins.CreateParameter();
            pAc.ParameterName = "$ac";
            ins.Parameters.Add(pAc);
            var pUc = ins.CreateParameter();
            pUc.ParameterName = "$uc";
            ins.Parameters.Add(pUc);
            var pVdd = ins.CreateParameter();
            pVdd.ParameterName = "$vdd";
            ins.Parameters.Add(pVdd);
            var pVt = ins.CreateParameter();
            pVt.ParameterName = "$vt";
            ins.Parameters.Add(pVt);
            var pCorn = ins.CreateParameter();
            pCorn.ParameterName = "$corn";
            ins.Parameters.Add(pCorn);
            var pEx = ins.CreateParameter();
            pEx.ParameterName = "$ex";
            ins.Parameters.Add(pEx);
            var pDecks = ins.CreateParameter();
            pDecks.ParameterName = "$decks";
            ins.Parameters.Add(pDecks);
            ...
            foreach (var (cls, count) in classDeviceCount)
            {
                ...
                string join(HashSet<string>? set, int take = 5) =>
                    set is null || set.Count == 0
                        ? string.Empty
                        : string.Join(
                            ',',
                            set.OrderBy(s => s, StringComparer.OrdinalIgnoreCase).Take(take)
                        );
    ...
    private static Dictionary<string, long> LoadIdMap(
    ...
    private static void UpsertLibraries(
        SqliteConnection conn,
        SqliteTransaction tx,
        IReadOnlyList<WorkspaceLibrary> libs,
    ...
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    private static void UpsertModels(
        SqliteConnection conn,
        SqliteTransaction tx,
    ...
    )
    {
        ...
        var mName = insertModel.CreateParameter();
        mName.ParameterName = "$name";
        insertModel.Parameters.Add(mName);
        var mType = insertModel.CreateParameter();
        mType.ParameterName = "$type";
        insertModel.Parameters.Add(mType);
        var mClass = insertModel.CreateParameter();
        mClass.ParameterName = "$class";
        insertModel.Parameters.Add(mClass);
        var mVdd = insertModel.CreateParameter();
        mVdd.ParameterName = "$vdd";
        insertModel.Parameters.Add(mVdd);
        var mVt = insertModel.CreateParameter();
        mVt.ParameterName = "$vt";
        insertModel.Parameters.Add(mVt);

        using var getId = conn.CreateCommand();
        getId.Transaction = tx;
        getId.CommandText = "SELECT id FROM models WHERE name=$name";
        var gName = getId.CreateParameter();
        gName.ParameterName = "$name";
        getId.Parameters.Add(gName);
        ...
        var sMid = insertSource.CreateParameter();
        sMid.ParameterName = "$model_id";
        insertSource.Parameters.Add(sMid);
        var sPath = insertSource.CreateParameter();
        sPath.ParameterName = "$path";
        insertSource.Parameters.Add(sPath);
        ...
        insertDeck.CommandText =
            ...
            VALUES ($model_id, $path)
            ON CONFLICT(model_id, path) DO NOTHING;";
        var dMid = insertDeck.CreateParameter();
        dMid.ParameterName = "$model_id";
        insertDeck.Parameters.Add(dMid);
        var dPath = insertDeck.CreateParameter();
        dPath.ParameterName = "$path";
        insertDeck.Parameters.Add(dPath);

        // store volts inline in models table; no auxiliary table

        // Dimension upserts for contexts
        using var insCornerTok = conn.CreateCommand();
        insCornerTok.Transaction = tx;
        insCornerTok.CommandText =
            "INSERT INTO corners(name) VALUES ($n) ON CONFLICT(name) DO NOTHING;";
        var pCornerName = insCornerTok.CreateParameter();
        pCornerName.ParameterName = "$n";
        insCornerTok.Parameters.Add(pCornerName);
        using var selCornerTok = conn.CreateCommand();
        selCornerTok.Transaction = tx;
        selCornerTok.CommandText = "SELECT id FROM corners WHERE name=$n";
        var pSelCorner = selCornerTok.CreateParameter();
        pSelCorner.ParameterName = "$n";
        selCornerTok.Parameters.Add(pSelCorner);

        using var insDetailTok = conn.CreateCommand();
        insDetailTok.Transaction = tx;
        insDetailTok.CommandText =
            "INSERT INTO details(name) VALUES ($n) ON CONFLICT(name) DO NOTHING;";
        var pDetailName = insDetailTok.CreateParameter();
        pDetailName.ParameterName = "$n";
        insDetailTok.Parameters.Add(pDetailName);
        using var selDetailTok = conn.CreateCommand();
        selDetailTok.Transaction = tx;
        selDetailTok.CommandText = "SELECT id FROM details WHERE name=$n";
        var pSelDetail = selDetailTok.CreateParameter();
        pSelDetail.ParameterName = "$n";
        selDetailTok.Parameters.Add(pSelDetail);

        using var insSectionTok = conn.CreateCommand();
        insSectionTok.Transaction = tx;
        insSectionTok.CommandText =
            "INSERT INTO sections(name) VALUES ($n) ON CONFLICT(name) DO NOTHING;";
        var pSectionName = insSectionTok.CreateParameter();
        pSectionName.ParameterName = "$n";
        insSectionTok.Parameters.Add(pSectionName);
        using var selSectionTok = conn.CreateCommand();
        selSectionTok.Transaction = tx;
        selSectionTok.CommandText = "SELECT id FROM sections WHERE name=$n";
        var pSelSection = selSectionTok.CreateParameter();
        pSelSection.ParameterName = "$n";
        selSectionTok.Parameters.Add(pSelSection);

        using var insIncludeTok = conn.CreateCommand();
        insIncludeTok.Transaction = tx;
        insIncludeTok.CommandText =
            "INSERT INTO includes(path) VALUES ($p) ON CONFLICT(path) DO NOTHING;";
        var pIncludePath = insIncludeTok.CreateParameter();
        pIncludePath.ParameterName = "$p";
        insIncludeTok.Parameters.Add(pIncludePath);
        using var selIncludeTok = conn.CreateCommand();
        selIncludeTok.Transaction = tx;
        selIncludeTok.CommandText = "SELECT id FROM includes WHERE path=$p";
        var pSelInclude = selIncludeTok.CreateParameter();
        pSelInclude.ParameterName = "$p";
        selIncludeTok.Parameters.Add(pSelInclude);
        ...
        var pMid = insContext.CreateParameter();
        pMid.ParameterName = "$mid";
        insContext.Parameters.Add(pMid);
        var pCid = insContext.CreateParameter();
        pCid.ParameterName = "$cid";
        insContext.Parameters.Add(pCid);
        var pDid = insContext.CreateParameter();
        pDid.ParameterName = "$did";
        insContext.Parameters.Add(pDid);
        var pSid = insContext.CreateParameter();
        pSid.ParameterName = "$sid";
        insContext.Parameters.Add(pSid);
        var pIid = insContext.CreateParameter();
        pIid.ParameterName = "$iid";
        insContext.Parameters.Add(pIid);
        ...
        foreach (var model in models)
        {
            ...
            var tokenForModel = VddFormatting.ExtractTokenFromVoltageDomain(
                model.VoltageDomain,
                matchingConfig
            );
            if (VddFormatting.TryTokenToVolts(tokenForModel, out var modelVolts))
                mVdd.Value = modelVolts;
            else
                mVdd.Value = DBNull.Value;
            ...
            if (contexts is not null)
            {
                foreach (var ctx in contexts)
                {
                    long? cornerId = null,
                        ...
                        sectionId = null,
                        includeId = null;
                    if (!string.IsNullOrWhiteSpace(ctx.Corner))
                    {
                        pCornerName.Value = ctx.Corner;
                        insCornerTok.ExecuteNonQuery();
                        pSelCorner.Value = ctx.Corner;
                        cornerId = (long?)selCornerTok.ExecuteScalar();
                    }
                    if (!string.IsNullOrWhiteSpace(ctx.Detail))
                    {
                        pDetailName.Value = ctx.Detail;
                        insDetailTok.ExecuteNonQuery();
                        pSelDetail.Value = ctx.Detail;
                        detailId = (long?)selDetailTok.ExecuteScalar();
                    }
                    if (!string.IsNullOrWhiteSpace(ctx.Section))
                    {
                        pSectionName.Value = ctx.Section;
                        insSectionTok.ExecuteNonQuery();
                        pSelSection.Value = ctx.Section;
                        sectionId = (long?)selSectionTok.ExecuteScalar();
                    }
                    if (!string.IsNullOrWhiteSpace(ctx.IncludePath))
                    {
                        pIncludePath.Value = Path.GetFullPath(ctx.IncludePath);
                        insIncludeTok.ExecuteNonQuery();
                        pSelInclude.Value = Path.GetFullPath(ctx.IncludePath);
                        includeId = (long?)selIncludeTok.ExecuteScalar();
                    }
    ...
    /// <param name="cancellationToken">Token to cancel the operation.</param>
    private static void UpsertDevices(
        SqliteConnection conn,
        SqliteTransaction tx,
        IReadOnlyList<Device> devices,
        CancellationToken cancellationToken
    )
    {
        ...
        insertDevice.CommandText =
            ...
            ON CONFLICT(canonical_name) DO UPDATE SET
                ...
                tags=excluded.tags;";
        var pKey = insertDevice.CreateParameter();
        pKey.ParameterName = "$key";
        insertDevice.Parameters.Add(pKey);
        var pDisplay = insertDevice.CreateParameter();
        pDisplay.ParameterName = "$display";
        insertDevice.Parameters.Add(pDisplay);
        var pLib = insertDevice.CreateParameter();
        pLib.ParameterName = "$lib";
        insertDevice.Parameters.Add(pLib);
        var pLibPath = insertDevice.CreateParameter();
        pLibPath.ParameterName = "$libpath";
        insertDevice.Parameters.Add(pLibPath);
        var pCell = insertDevice.CreateParameter();
        pCell.ParameterName = "$cell";
        insertDevice.Parameters.Add(pCell);
        var pCellPath = insertDevice.CreateParameter();
        pCellPath.ParameterName = "$cellpath";
        insertDevice.Parameters.Add(pCellPath);
        var pClass = insertDevice.CreateParameter();
        pClass.ParameterName = "$class";
        insertDevice.Parameters.Add(pClass);
        var pSubclass = insertDevice.CreateParameter();
        pSubclass.ParameterName = "$subclass";
        insertDevice.Parameters.Add(pSubclass);
        var pLayout = insertDevice.CreateParameter();
        pLayout.ParameterName = "$layout";
        insertDevice.Parameters.Add(pLayout);
        var pSymbol = insertDevice.CreateParameter();
        pSymbol.ParameterName = "$symbol";
        insertDevice.Parameters.Add(pSymbol);
        var pVt = insertDevice.CreateParameter();
        pVt.ParameterName = "$vt";
        insertDevice.Parameters.Add(pVt);
        var pVdd = insertDevice.CreateParameter();
        pVdd.ParameterName = "$vdd";
        insertDevice.Parameters.Add(pVdd);
        var pTags = insertDevice.CreateParameter();
        pTags.ParameterName = "$tags";
        insertDevice.Parameters.Add(pTags);

        using var getId = conn.CreateCommand();
        getId.Transaction = tx;
        getId.CommandText = "SELECT id FROM devices WHERE canonical_name=$key";
        var gKey = getId.CreateParameter();
        gKey.ParameterName = "$key";
        getId.Parameters.Add(gKey);
        ...
        var vId = insertView.CreateParameter();
        vId.ParameterName = "$id";
        insertView.Parameters.Add(vId);
        var vView = insertView.CreateParameter();
        vView.ParameterName = "$view";
        insertView.Parameters.Add(vView);
        ...
        foreach (var d in devices)
        {
            ...
            {
                if (VddFormatting.TryTokenToVolts(t, out var v))
                {
                    voltsValue = v;
                    break;
                }
