...
1. Provide complete geometric specification. Every construct expands deterministically to GDS-II/OASIS primitives. Given an ACIR-LL document and PDK binding, the output is bit-identical across runs.
...
Deterministic expansion: Given an ACIR-LL document, PDK binding, and P-cell library version, the output GDS is bit-identical across implementations and runs.
...
ACIR-LL references PDK constructs by logical name. The PDK binding provides the mapping to physical data:

| Component | Description | Example |
|-----------|-------------|---------|
| Layer map | Logical name -> GDS layer/datatype | `m1` -> `68/20` |
...
| Via definitions | Via cut geometry and enclosures | `via1: cut=68/44, size=150n` |
| Database unit | Physical size of one GDS unit | `1nm` |
| Manufacturing grid | Minimum coordinate resolution | `5nm` |

The PDK binding is a separate artifact loaded at expansion time. ACIR-LL documents are specific to a particular PDK and PDK version.
...
Layer names are identifiers that map to GDS layer/datatype pairs via the PDK binding:
...
m1, m2, m3, m4, m5              // metals
mcon, via1, via2, via3, via4    // vias/contacts
```

Layer names are logical identifiers. The PDK binding defines their physical GDS layer/datatype mapping.
...
Required. Specifies which PDK binding and version to use for expansion.

The expander resolves this to a PDK binding file. If the PDK binding cannot be found or the version is incompatible, expansion fails with an error.
...
If omitted, the PDK binding's default is used.
...
All coordinates are rounded to the nearest multiple of this value before conversion to database units. If omitted, the PDK binding's default is used.
...
| `pcell_name` | PDK P-cell name as defined in the PDK binding |
...
1. The expander validates that `pcell_name` exists in the PDK binding.
...
4. The PDK's P-cell generator is invoked with the specified parameters.
...
The `W` parameter specifies width per finger. For multi-finger devices (NF > 1), total device width is `W * NF`. This convention is fixed by ACIR-LL; PDK bindings must normalize to this interpretation.
...
1. The coordinate expression is evaluated.
2. The expander looks up `via_type` in the PDK binding.
...
## 4. PDK Binding Specification

### 4.1 Overview

The PDK binding maps ACIR-LL's logical constructs to physical GDS data. It is a YAML file loaded by the expander.

File naming convention: `<pdk_name>-<version>.acir-pdk.yml`

### 4.2 Top-Level Structure

```yaml
pdk:
  name: <string>              # Must match ACIR-LL pdk declaration
  version: <string>           # PDK binding version
  foundry: <string>           # Foundry name (informational)
  technology: <string>        # Technology node (informational)

database:
  dbu: <physical>             # Default database unit
  grid: <physical>            # Default manufacturing grid

layers:
  <layer_name>: { gds: [<layer>, <datatype>], purpose: <purpose> }
  ...

vias:
  <via_name>:
    cut_layer: [<layer>, <datatype>]
    cut_size: [<width>, <height>]
    cut_spacing: [<x_space>, <y_space>]
    lower_layer: <layer_name>
    upper_layer: <layer_name>
    enclosure_lower: [<x_enc>, <y_enc>]
    enclosure_upper: [<x_enc>, <y_enc>]
  ...
...
pcells:
  <pcell_name>:
    library: <string>
    cell: <string>
    origin: [<x>, <y>]        # P-cell origin point
    parameters:
      <param_name>: { type: <type>, range: [<min>, <max>], default: <value> }
      ...
    anchors:
      <anchor_name>: { expr: "<expression>" }
      ...
    pins:
      <pin_name>: { layer: <layer_name>, anchor: <anchor_name> }
      ...
  ...
```
...
layers:
  # Drawing layers
  diff:     { gds: [65, 20], purpose: drawing }
  poly:     { gds: [66, 20], purpose: drawing }
  li:       { gds: [67, 20], purpose: drawing }
  m1:       { gds: [68, 20], purpose: drawing }
  m2:       { gds: [69, 20], purpose: drawing }
  m3:       { gds: [70, 20], purpose: drawing }
  m4:       { gds: [71, 20], purpose: drawing }
  m5:       { gds: [72, 20], purpose: drawing }
  
  # Well and implant layers
  nwell:    { gds: [64, 20], purpose: drawing }
  pwell:    { gds: [64, 44], purpose: drawing }
  nplus:    { gds: [65, 49], purpose: implant }
  pplus:    { gds: [65, 48], purpose: implant }
  
  # Pin layers (for ports)
  m1.pin:   { gds: [68, 16], purpose: pin }
  m2.pin:   { gds: [69, 16], purpose: pin }
  m3.pin:   { gds: [70, 16], purpose: pin }
  m4.pin:   { gds: [71, 16], purpose: pin }
  
  # Label layers
  m1.label: { gds: [68, 5], purpose: label }
  m2.label: { gds: [69, 5], purpose: label }
  m3.label: { gds: [70, 5], purpose: label }
  m4.label: { gds: [71, 5], purpose: label }
```

Purpose values: `drawing`, `pin`, `label`, `boundary`, `blockage`
...
vias:
  mcon:
    cut_layer: [66, 44]
    cut_size: [170n, 170n]
    cut_spacing: [170n, 170n]
    lower_layer: li
    upper_layer: m1
    enclosure_lower: [60n, 30n]
    enclosure_upper: [30n, 60n]
  ...
  via1:
    ...
    lower_layer: m1
    upper_layer: m2
    enclosure_lower: [55n, 85n]
    enclosure_upper: [55n, 85n]
    
  via2:
    cut_layer: [69, 44]
    cut_size: [200n, 200n]
    cut_spacing: [200n, 200n]
    lower_layer: m2
    upper_layer: m3
    enclosure_lower: [40n, 85n]
    enclosure_upper: [65n, 65n]
  ...
  via3:
    ...
    lower_layer: m3
    upper_layer: m4
    enclosure_lower: [60n, 60n]
    enclosure_upper: [60n, 60n]
```

Via expansion algorithm:

For a single via of type `V` at center `(cx, cy)`:
1. Emit cut rectangle centered at `(cx, cy)` with dimensions from `cut_size`
2. Emit lower enclosure: cut bbox expanded by `enclosure_lower`
3. Emit upper enclosure: cut bbox expanded by `enclosure_upper`

### 4.5 P-Cell Definitions with Anchors
...
pcells:
  nfet_01v8:
    ...
    cell: nfet_01v8
    origin: [0, 0]            # Origin at lower-left of active
    ...
    anchors:
      # Anchors are expressions evaluated with P-cell parameters
      origin:     { expr: "[0, 0]" }
      gate:       { expr: "[L/2 + 130n, W/2]" }
      drain:      { expr: "[L + 260n, W/2]" }
      source:     { expr: "[0, W/2]" }
      body:       { expr: "[L/2 + 130n, -200n]" }
      bbox.ll:    { expr: "[-130n, -200n]" }
      bbox.ur:    { expr: "[L + 390n, W + 200n]" }
      bbox.center: { expr: "[(L + 260n)/2, W/2]" }
      
      # For multi-finger devices, additional anchors
      drain.0:    { expr: "[L + 260n, W/2]" }
      source.0:   { expr: "[0, W/2]" }
      # ... pattern continues based on NF
    
    pins:
      G: { layer: poly, anchor: gate }
      D: { layer: m1, anchor: drain }
      S: { layer: m1, anchor: source }
      B: { layer: li, anchor: body }
```

Anchor expression evaluation:

Anchor coordinates are computed from P-cell parameters at expansion time:
1. Parameter values are substituted into the expression
2. Expression is evaluated to `[x, y]` in physical units
3. Result is transformed by placement orientation
4. Result is translated by placement coordinate

Example:
```
place M1 nfet_01v8 (W=1u, L=180n, NF=1) @ (5u, 10u) R0
```

Evaluating `drain` anchor:
1. Expression: `[L + 260n, W/2]`
2. Substitution: `[180n + 260n, 1u/2]` = `[440n, 500n]`
3. Orientation R0: `[440n, 500n]` (unchanged)
4. Translation: `[5u + 440n, 10u + 500n]` = `[5.44u, 10.5u]`

Therefore `M1@drain` = `(5.44u, 10.5u)`

### 4.6 P-Cell Origin Convention

Requirement: Every P-cell definition must specify its origin point. The origin is the point about which orientation transformations are applied.

Recommended convention: Origin at the lower-left corner of the active region (diffusion for transistors).

This ensures predictable behavior when mirroring devices for interleaved layouts.

### 4.7 Advanced Node Extensions

FinFET PDK binding additions:
  ...
  nfet:
    parameters:
      NFIN:
        type: int
        range: [1, 8]
        description: "Number of fins (replaces W)"
      L:
        type: real
        values: [7n, 16n, 20n]    # Discrete lengths only
      VT:
        type: enum
        values: [LVT, SVT, HVT]
```

GAA PDK binding additions:
...
nanosheet:
  stack_height: 3
  sheet_width_range: [10n, 50n]

backside:
  enabled: true
  power_rail_layer: { gds: [100, 20] }
  power_rail_pitch: 48n
```
...
doc -> parser -> consts -> anchors -> validate -> expand -> writer -> gds
```
...
1. Load PDK bindings by name and version.
2. Reject version mismatches between document and available PDK.
3. Resolve all layer names to GDS layer/datatype pairs.
4. Validate P-cell parameters against PDK-specified ranges.
5. Compute anchor positions using PDK-defined expressions.
6. Instantiate P-cells via the PDK's mechanism.
7. Expand vias according to PDK definitions.
...
The PDK binding may reference proprietary P-cell libraries. Implementations must respect foundry licensing terms.
...
## Appendix A: Example PDK Binding (sky130)
...
layers:
  diff:     { gds: [65, 20], purpose: drawing }
  tap:      { gds: [65, 44], purpose: drawing }
  nwell:    { gds: [64, 20], purpose: drawing }
  pwell:    { gds: [64, 13], purpose: drawing }
  nplus:    { gds: [65, 49], purpose: implant }
  pplus:    { gds: [65, 48], purpose: implant }
  poly:     { gds: [66, 20], purpose: drawing }
  li:       { gds: [67, 20], purpose: drawing }
...
pcells:
  nfet_01v8:
    ...
    parameters:
      W:  { type: real, range: [420n, 50u], default: 420n, 
            description: "Channel width per finger" }
      L:  { type: real, range: [150n, 50u], default: 150n }
      NF: { type: int, range: [1, 50], default: 1 }
      M:  { type: int, range: [1, 50], default: 1 }
  ...
  pfet_01v8:
    ...
    anchors:
      ...
      drain:       { expr: "[L + 260n, W/2]" }
      source:      { expr: "[0, W/2]" }
      body:        { expr: "[L/2 + 130n, W + 200n]" }
      bbox.ll:     { expr: "[-130n, -200n]" }
      bbox.ur:     { expr: "[L + 390n, W + 400n]" }
      bbox.center: { expr: "[(L + 260n)/2, W/2]" }
  ...
  tfr:
    ...
    parameters:
      R: { type: real, range: [100, 10M] }
      W: { type: real, range: [500n, 50u], default: 2u }
      L: { type: real, range: [1u, 500u] }
    ...
    pins:
      P: { layer: m1, anchor: plus }
      N: { layer: m1, anchor: minus }
  ...
  mim_cap:
    ...
    parameters:
      C: { type: real, range: [1f, 50p] }
    
    anchors:
      origin:  { expr: "[0, 0]" }
      plus:    { expr: "[size/2, size/2]", note: "size computed from C" }
      minus:   { expr: "[size/2, size/2]" }
      bbox.ll: { expr: "[0, 0]" }
      bbox.ur: { expr: "[size, size]" }
